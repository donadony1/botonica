...
export default function ProductsTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#9aad98] text-sm mt-1">
            {products.length} produit{products.length > 1 ? 's' : ''} — Conformité GPSR & Suivi des stocks
          </p>
        </div>
        <button
          onClick={handleAdd}
      ...
      {/* Products Table */}
      <div className="bg-[#1e2a1e] border border-[#2d3d2c] rounded-2xl overflow-hidden shadow-xl">
        {filtered.length === 0 ? (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#2d3d2c]">
                  ...
                  <th className="text-left px-4 py-4 text-[#9aad98] text-xs font-semibold uppercase tracking-wider">Stock</th>
                  <th className="text-left px-4 py-4 text-[#9aad98] text-xs font-semibold uppercase tracking-wider hidden sm:table-cell">GPSR / INCI</th>
                ...
                {filtered.map(product => {
                  ...
                  return (
                    <tr key={product.id} className="hover:bg-[#243023] transition-colors duration-150">
                      ...
                      <td className="px-4 py-4">
                        {isOut ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-red-900/40 text-red-400 border border-red-800/50">
                            <span className="w-1.5 h-1.5 rounded-full bg-red-400"></span> 0 (Rupture)
                          </span>
                        ) : isLow ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-amber-900/40 text-amber-300 border border-amber-800/50">
                            <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-ping"></span> {product.stock} (Alerte bas)
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-950/40 text-emerald-300 border border-emerald-800/50">
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400"></span> {product.stock}
                          </span>
                        )}
                      </td>
                      <td className="px-4 py-4 hidden sm:table-cell">
                        {product.inci ? (
                          <span className="text-emerald-400 text-xs font-medium flex items-center gap-1">
                            <span className="material-symbols-outlined text-[16px]">verified</span> Conforme
                          </span>
                        ) : (
                          <span className="text-amber-400 text-xs font-medium flex items-center gap-1">
                            <span className="material-symbols-outlined text-[16px]">warning</span> INCI manquante
                          </span>
                        )}
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            className="p-2 text-[#9aad98] hover:text-white hover:bg-[#2a3529] rounded-lg transition-all duration-150 cursor-pointer"
                            title="Modifier"
                          >
                          ...
                          <button
                            ...
                            className="p-2 text-[#9aad98] hover:text-red-400 hover:bg-red-900/20 rounded-lg transition-all duration-150 cursor-pointer"
                            title="Supprimer"
                          >
          ...
          <div className="bg-[#1e2a1e] border border-[#2d3d2c] rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-4 py-2.5 rounded-xl border border-[#3d4f3c] text-[#9aad98] hover:text-white hover:border-[#bb0a4a] transition-all cursor-pointer"
              >
                Annuler
              </button>
              <button
                onClick={() => handleDelete(deleteConfirm)}
                className="flex-1 px-4 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white font-semibold transition-all cursor-pointer"
              >
                Supprimer
              </button>
            </div>

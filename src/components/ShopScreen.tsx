      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12">
          ...
          {tags.map((tag) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs uppercase tracking-wider font-semibold transition-all duration-200 cursor-pointer ${selectedTag === tag.id
                ? 'bg-[#1a191c] text-white shadow-sm'
        ...
        <div className="relative w-full md:w-72">
          <input
            ...
            className="w-full bg-white border border-[#c4c8c0]/60 rounded-full pl-10 pr-4 py-2.5 text-sm text-[#1a1c1c] placeholder:text-[#747871] focus:outline-none focus:border-[#1a191c] focus:ring-1 focus:ring-[#1a191c] transition-all"
          />
      ...
      {filteredProducts.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-3xl border border-[#c4c8c0]/30 p-8">
          ...
          <button
            ...
            className="px-6 py-2.5 bg-[#1a191c] text-white rounded-full text-xs uppercase font-semibold tracking-wider"
          >
            ...
            >
              ...
              <div>
                ...
                >
                  ...
                  {product.surgrasPercentage && (
                    <span className="absolute bottom-3 right-3 bg-[#f9f9f9]/90 text-[#1a191c] text-[11px] font-semibold px-2.5 py-1 rounded-full backdrop-blur-xs">
                ...
                <h3
                  onClick={() => onSelectProduct(product)}
                  className="font-serif-luxury text-2xl text-[#1a1c1c] mb-2 hover:text-[#1a191c] transition-colors cursor-pointer"
                >
                  {product.name}
                </h3>
              ...
              <div className="pt-4 border-t border-[#f3f3f4]">
                <div className="flex justify-between items-baseline mb-4">
                  <span className="font-serif-luxury text-2xl text-[#1a191c] font-semibold">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={() => onSelectProduct(product)}
                    className="py-2.5 px-3 border border-[#1a191c] text-[#1a191c] rounded-full text-xs font-semibold uppercase tracking-wider hover:bg-[#e2e0d7] transition-colors text-center cursor-pointer"
                  >
                    Détails
                  </button>
                  <button
                    onClick={() => onAddToCart(product, 1)}
                    className="py-2.5 px-3 bg-[#1a191c] text-white rounded-full text-xs font-semibold uppercase tracking-wider hover:bg-[#b7003a] transition-colors flex items-center justify-center gap-1 cursor-pointer"
                  >
